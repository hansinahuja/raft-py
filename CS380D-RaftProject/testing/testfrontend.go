...
// Function to start one specific server
func (ts *TestingStruct) StartServer(numServers int) bool {
	req := &pb.IntegerArg{
	...
	res, err := client.StartServer(ctx, req)
	if err != nil || res.GetError() != "" {
		fmt.Printf("could not start raft: %v \n", err)
		return false
	}
	time.Sleep(1 * time.Second) // Starting up a server takes more time than lifting network partition, so we added a timeout here
	return true
...
func (ts *TestingStruct) DisconnectLeader() bool {
	...
	if !ok {
		fmt.Println("DisconnectLeader: Failed to get leader")
		ok := ts.StartServer(leader)
		if !ok {
			fmt.Printf("Error while starting leader. \n")
		}
		return false
	}
	fmt.Printf("Leader after blocking ports is %v \n", newleader)

	start_ok := ts.StartServer(leader)
	if !start_ok {
		fmt.Printf("Error while starting leader again. \n")
	}
...
func (ts *TestingStruct) DisconnectMinority() bool {
	...
	for serverToDisconnect := 0; serverToDisconnect < nodesToDisconnect; serverToDisconnect++ {
		ts.KillOneNode(serverToDisconnect)
	}
	...
	for serverToDisconnect := 0; serverToDisconnect < nodesToDisconnect; serverToDisconnect++ {
		ok := ts.StartServer(serverToDisconnect)
		fmt.Printf("Started again\n")
		if !ok {
			fmt.Printf("Error while starting %v again. \n", serverToDisconnect)
			return false
		}
	}
